...
if($Wcms->currentPage == $Wcms->get('config', 'login')) {
    ...
    $type = isset($Wcms->get('pages', $Wcms->currentPage)->here2there) ? $Wcms->get('pages', $Wcms->currentPage)->here2there : "dual";
...
<!DOCTYPE html>
<!--
	WonderCMS 3.0.0 here2there Theme
	by Stephan Stanisic
-->
<html lang="en">
<head>
	...
	<style>.here2there{height:<?=htmlentities($height)?>vh;}</style>
</head>
<body>
        <?= $Wcms->settings() ?>
        <?= $Wcms->alerts() ?>
		...
		<div class="container">
			<div class="navbar-header">
				...
				<a class="navbar-brand" href="<?=$Wcms->url()?>">
					...
					<img src="images/here-2-there-white.png" title="Here2There.Me" />
				</a>
			</div>
    ...
    <header class="here2there-wrapper">
        <div class="here2there" style='background-image: url(<?=json_encode($page_image)?>);'>
            <div class="inner">
                <h1><?= $heading ?></h1>
    ...
    <style>.here2there .scrolly { display: none }</style>
    ...
    <?php /* Add some overwrites for pages with different here2there types. */ ?>
    <?php if($type == "scroll") echo "<style>.here2there{background-attachment:fixed;}</style>"; ?>
    <?php if($height == 100) echo "<style>.here2there{padding-top:0;}</style>"; ?>
</body>
</html>
